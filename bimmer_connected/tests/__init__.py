...
ALL_CHARGING_SESSIONS_DETAIL: Dict[str, Dict] = {}
...
for charging_sessions in RESPONSE_DIR.rglob("*-eadrax-chs_v1_charging-sessions_*.json"):
    vin = charging_sessions.stem.split("_")[-1]
    session_data = load_response(charging_sessions)

    if "chargingSessions" in session_data:
        ALL_CHARGING_SESSIONS.setdefault(vin, {}).setdefault("root", session_data)
    else:
        ALL_CHARGING_SESSIONS.setdefault(vin, {}).setdefault("details", {}).setdefault(session_data["id"], session_data)
